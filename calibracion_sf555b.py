# -*- coding: utf-8 -*-
"""
Calibracion SF555B - toma muestras cada 2 s y exporta a Excel
para comparar con el sonometro y calcular el ajuste.
"""
...
def exportar_xlsx(muestras, archivo):
    ...
    # Encabezados
    encabezados = ["#", "Hora", "Mic (dBFS)", "Sonometro (dB)", "Diferencia (dB)"]
    ...
    for fila, (ts, db_mic) in enumerate(muestras, 2):
        ...
        ws.cell(row=fila, column=4, value=None)   # el usuario llena el sonometro
        # Sonometro - Mic
    ...
    n = len(muestras) + 1
    fila_prom = n + 2
    ws.cell(row=fila_prom, column=2, value="PROMEDIO OFFSET").font = Font(bold=True)  # noqa
    ...
    wb.save(archivo)
    print(f"\n[OK] Guardado: {archivo}")
    print(f"     Rellena la columna 'Sonometro (dB)' con tus lecturas.")
...
def main():
    ...
    try:
        ...
        print(f"\n--- {len(muestras)} muestras capturadas ---")  # noqa
    finally:
        stream.stop()
        stream.close()
